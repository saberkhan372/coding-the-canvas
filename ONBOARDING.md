...
<head>
  ...
  <title>[Page Title] - CS Canvas</title>
...
<body data-page="[section]">   <!-- teachers | about | browse | etc. -->
  <header class="site-header">
    <a class="brand" href="[path]/index.html">CS Canvas</a>
    <nav>
...
- `teachers/ap-csa/` is mostly aligned to the current concept map, but still has minor terminology cleanup:
  - `12–15 §5` should become `11–16 §5`
  - `Histograms` should consistently become `Histograms & Sampling`
  - The `Binary as Pixels` U8 explanation may need wording that better matches the concept
...
All 10 teacher pages, the AP CSA concept reference fixes, bridge/tool footer fixes, `data-page="browse"` updates, generator patch, and handoff docs are currently local/uncommitted. Review and commit before starting larger new work.

### Priority 2 — Finish AP CSA terminology cleanup
The broken AP CSA link is fixed and most stale names are gone. Finish the small leftovers listed above.

### Priority 3 — Flesh out bridge pages (if desired)
...
### Priority 4 — Flesh out tool pages (if desired)
