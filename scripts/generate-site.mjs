...
function writeHome() {
  ...
  `;
  write('index.html', shell({ path: 'index.html', title: 'Coding the Canvas', kicker: 'visual CS atlas', description: 'A complete static-page scaffold for the CS Canvas curriculum, guided by the Claude wireframes.', body, active: 'home' }));
}
...
function writeSimpleCollection(base, title, description, items, active) {
  ...
  write(indexPath, shell({ path: indexPath, title, kicker: base, description, body, active }));
  for (const [slug, itemTitle, itemDesc, needs] of items) {
    ...
    write(path, shell({ path, title: itemTitle, kicker: title, description: itemDesc, body: bodyDetail, active }));
  }
}
...
function writeCorePages() {
  for (const [path, title, description, needs] of corePages) {
    ...
    write(path, shell({ path, title, kicker: 'site page', description, body, active }));
  }
}
